	...
	public static void ExportTeleporters()
    {
        ...
        foreach (var map in DataManager.Instance.Maps)
        {
            ...
            foreach(var teleporter in teleporters)
            {
                ...
                {
                    EditorUtility.DisplayDialog("错误", string.Format("地图:{0} 中配置得 Teleporter:[{1}]中不存在", map.Value.Resource, teleporter.ID), "确定");
                    return;
                }
                ...
                {
                    EditorUtility.DisplayDialog("错误", string.Format("地图:{0} 中配置得 Teleporter:[{1}] MapID:{2} 错误", map.Value.Resource, teleporter.ID, def.MapID), "确定");
                    return;
                }
